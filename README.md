...
- **Input Monitor and Test Tone** — collapsed by default; microphone gain
  boost and a live level meter, plus a pure tone (with its own frequency,
  fine-tuning and volume controls) fed straight into the tuner and the
  speakers, no mic needed — the same panel /strobetuner/ and /multistrobe/
  use too
